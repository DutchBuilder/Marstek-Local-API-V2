...
def _float_entity(hass: HomeAssistant, entity_id: str | None, default: float) -> float:
    if not entity_id:
        return default
...
def compute_plan(
    ...
    *,
    ...
    Returns a dict with keys:
        ...
        charge_hours, discharge_hours, charge_watts_per_bat, discharge_watts_per_bat,
    ...
    eb = _float_entity(hass, tax_entity, 0.0)
    fee = _float_entity(hass, fee_entity, 0.0)
    n = int(_float_entity(hass, plan_hours_entity, 6.0))
    min_spread = _float_entity(hass, min_spread_entity, 0.062)
    btw = 1.21
    ...
    for x in prices_raw:
        try:
            ...
            if st.date() == target_date:
                p = (float(x.get("price", 0)) + eb + fee) * btw
    ...
    watts_total = watts_per_bat * num_batteries
    kwh_per_hour_full = watts_total / 1000

    if spread_ok:
        strategy = "WINSTGEVEND"
        charge_hours = n
        discharge_hours = n
        discharge_watts_per_bat = watts_per_bat
    else:
        ...
        charged_kwh = min(n * kwh_per_hour_full, capacity_kwh)
        needed_w_per_bat = (charged_kwh / discharge_hours * 1000) / num_batteries
        discharge_watts_per_bat = min(int(needed_w_per_bat), watts_per_bat)
    ...
    charge_total_kwh = min(charge_hours * kwh_per_hour_full, capacity_kwh)
    ...
    buy_cost = sum(it["p"] * kwh_per_hour_full for it in ch2)
    ...
    return {
        ...
        "charge_watts_per_bat": watts_per_bat,
...
def is_current_hour_in_slots(slots: list[dict]) -> bool:
    ...
    now = datetime.now()
    now_hhmm = now.replace(minute=0, second=0, microsecond=0).strftime("%H:%M")
    for slot in slots:
        if slot.get("start") == now_hhmm:
            return True
    return False
